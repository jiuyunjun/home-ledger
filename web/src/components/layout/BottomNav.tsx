...
interface VoiceEntryResult {
  mode?: 'candidates';        // present when multiple items were detected
...
const CANCEL_THRESHOLD = 60; // px upward movement to trigger cancel

function VoicePlusButton() {
  const router = useRouter();
  const [voiceState,    setVoiceState]    = useState<VoiceState>('idle');
  const [bars,          setBars]          = useState<number[]>(Array(BAR_COUNT).fill(0));
  const [willCancel,    setWillCancel]    = useState(false); // finger moved up enough
  ...
  function startViz(stream: MediaStream) {
    ...
    function tick() {
      ...
      setBars(BIN_INDICES.map((bin) => {
        const v = data[Math.min(bin, data.length - 1)] / 255;
        return Math.round(8 + v * 28);
      }));
  ...
  function handlePointerMove(e: React.PointerEvent) {
    ...
    const deltaY = startYRef.current - e.clientY; // positive = moved up
    setWillCancel(deltaY > CANCEL_THRESHOLD);
  }
  ...
  const barColor     = willCancel ? '#6B7280' : '#DC2626';
  const btnBg        = isRecording ? (willCancel ? '#6B7280' : '#DC2626') : T.accent;

  return (
    <>
      <style>{`
        @keyframes voiceSpin    { to { transform: rotate(360deg); } }
        @keyframes voiceBarIdle { 0%,100% { height:8px; } 50% { height:14px; } }
      `}</style>

      {/* Overlay card — visible above the finger */}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{
              ...
              background: isProcessing ? T.accentSoft : (willCancel ? '#F3F4F6' : '#FEE2E2'),
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: isProcessing ? T.textSoft : (willCancel ? '#6B7280' : '#DC2626') }}>
              {isProcessing ? 'AI 识别中…' : willCancel ? '松开取消' : '松开停止'}
            </span>
          </div>
          ...
          {isProcessing && (
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 36 }}>
              {Array(BAR_COUNT).fill(0).map((_, i) => (
                <div key={i} style={{
                  width: 4, borderRadius: 2, background: T.accent, opacity: 0.4,
                  animation: `voiceBarIdle 1.2s ease-in-out ${i * 0.15}s infinite`, height: 10,
                }} />
              ))}
            </div>
          )}
      ...
      <div
        ...
        style={{
          ...
          boxShadow: isRecording
            ? `0 0 0 6px ${willCancel ? 'rgba(107,114,128,0.18)' : 'rgba(220,38,38,0.18)'}, 0 4px 12px ${willCancel ? 'rgba(107,114,128,0.3)' : 'rgba(220,38,38,0.4)'}`
        ...
        return (
          <Link key={it.id} href={it.href} style={{ textDecoration: 'none' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, color: on ? T.accent : T.textMute, width: 56 }}>
              <div style={{ width: 22, height: 22, borderRadius: 6, background: on ? T.accentSoft : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 600 }}>{it.icon}</div>
